...
const createRows = (jobs: string[]) => {
    ...
    for (let i = 0; i < rowCount; i++) {
        const start = Math.floor(i * jobs.length / rowCount);
        const end = Math.floor((i + 1) * jobs.length / rowCount);
        parts.push(jobs.slice(start, end));
    }
...
function ScrollingRow({ jobs, speed, rowIndex }: ScrollingRowProps) {
...
export default function HomeCompanySlider({ jobs }: HomeJobSliderProps) {
    const [rows] = useState(() => createRows(jobs));
    const speeds = [38, 40];
